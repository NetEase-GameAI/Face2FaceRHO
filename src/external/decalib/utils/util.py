...
def calculate_scale_tx_ty(cam, tform, image_size, original_image_size):
    batch_size = cam.shape[0]
    device = cam.device

    trans_matrixes = []
    trans1 = torch.zeros((batch_size, 4, 4), dtype=torch.float32, device=device)
    trans1[:, 0, 0] = trans1[:, 1, 1] = trans1[:, 2, 2] = cam[:, 0]
    trans1[:, 1, 1] = -trans1[:, 1, 1]
    trans1[:, 2, 2] = -trans1[:, 2, 2]
    trans1[:, 0, 3] = cam[:, 1] * cam[:, 0]
    trans1[:, 1, 3] = cam[:, 2] * cam[:, 0]
    trans1[:, 1, 3] = -trans1[:, 1, 3]
    trans1[:, 3, 3] = 1
    trans_matrixes.append(trans1)

    trans2 = torch.zeros((batch_size, 4, 4), dtype=torch.float32, device=device)
    trans2[:, 0, 0] = trans2[:, 1, 1] = image_size / 2
    trans2[:, 2, 2] = 1
    trans2[:, 0, 3] = trans2[:, 1, 3] = image_size / 2
    trans2[:, 3, 3] = 1
    trans_matrixes.append(trans2)

    trans3 = torch.zeros((batch_size, 4, 4), dtype=torch.float32, device=device)
    tform_inv = torch.inverse(tform)
    trans3[:, 0:2, 0:2] = tform_inv[:, 0:2, 0:2]
    trans3[:, 0:2, 3] = tform_inv[:, 0:2, 2]
    trans3[:, 2, 2] = 1
    trans3[:, 3, 3] = 1
    trans_matrixes.append(trans3)

    trans4 = torch.zeros((batch_size, 4, 4), dtype=torch.float32, device=device)
    trans4[:, 0, 0] = trans4[:, 1, 1] = 2 / original_image_size
    trans4[:, 2, 2] = 1
    trans4[:, 0, 3] = trans4[:, 1, 3] = -1
    trans4[:, 3, 3] = 1
    trans_matrixes.append(trans4)

    full_trans_matrix = torch.eye(4, device=device).unsqueeze(0).expand(batch_size, -1, -1)

    for matrix in trans_matrixes:
        full_trans_matrix = torch.bmm(matrix, full_trans_matrix)

    scale = full_trans_matrix[:, 0, 0]
    sz = full_trans_matrix[:, 2, 2]
    tx = full_trans_matrix[:, 0, 3]
    ty = full_trans_matrix[:, 1, 3]

    return scale, tx, ty, sz

def calculate_crop_scale_tx_ty(cam):
    batch_size = cam.shape[0]
    device = cam.device
